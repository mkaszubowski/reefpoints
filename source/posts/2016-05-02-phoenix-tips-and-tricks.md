...
Often times, you'll find yourself repeatedly needing to access connection information in your controller actions, such as `conn.assigns.current_user` or similarly reaching deeply into nested connection information. This can get tedious and obscure the code. While we could extract the lookup to a function, such as `current_user(conn)`, then we are needlessly doing extra map access when we only need to do the lookup once. There's a better way.

Phoenix controllers all contain and `action/2` plug, which is called last in the controller pipeline. This function is responsible for calling the function specified in the route, but Phoenix makes it overridable so you can customized your controller actions. For example, imagine the following controller:
